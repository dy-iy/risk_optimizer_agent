...
# score_whale 开平仓/头寸调整现在不再完全排除，改为减权
NEG_WHALE_POSITION = []  # 清空，不再使用
...
# 1) 合约/链上漏洞攻击 (patch v2.2 强化抑制并补充漏召回)
def score_hack(text):
    # 商业无关新闻排除，除非存在明确攻击动作
...
# 3) 监管/法律风险 (patch v2.2 细化并降低泛化触发)
...
def score_regulatory(text):
    ...
    if has_crypto and has_reg_signal:
        # 若同时有强执法关键词
        if has_any(text, ["法案", "制裁", "调查", "起诉", "指控", "草案", "国会批评", "监管阻力"]):
            return 0.40
        # 仅有一般监管词
...
# 6) 清算/爆仓 (patch v2.2 收紧触发并排除产品/策略)
...
# 7) 大额转账/巨鲸 (patch v2.2 大幅改善漏召，移除过分排除)
...
def score_whale(text):
    ...
    if not has_any(text, ["转入", "转出", "转移", "发送", "从地址", "链上", "地址", "巨鲸", "whale", "大户", "鲸鱼"] + KW_WHALE_BEHAVIOUR):
        return 0.0
    ...
    if has_any(text, KW_WHALE_BEHAVIOUR):
        base = 0.55
        if usd > 0:
            base = clip01(0.55 + 0.20 * smooth_strength(usd, 1_000_000, 50_000_000))  # 降低门槛至100万
        return base * future_discount * position_discount

    # 弱信号无法再判断，现在都作为强信号处理，但提高小额门槛到 1M
    if usd <= 1_000_000:
...
# 8) 行情异常波动 (patch v2.2 扩展波动词并限制非跌势)
...
def score_volatility(text):
    ...
    if has_any(text, KW_VOL_MISS):
        base = 0.40 if has_softener else 0.55  # 提升基础分
        if has_time:
            base += 0.05
...
# 12) 宏观 / 政策冲击风险 (patch v2.2 强制加密市场关联)
