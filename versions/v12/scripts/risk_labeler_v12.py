...
# score_regulatory 新增：否定/缓和词（质疑、反对等），无强负面时高分封顶
...
# 3) 监管/法律风险 (patch v2.4 增加缓和否定词，非制裁类封顶0.4，弱化讨论信号)
...
def score_regulatory(text):
    ...
    if has_any(text, REG_ACTORS) and has_strong:
        base = 0.80
        if discuss_discount:
            base = min(base, 0.4)  # 存在反对/质疑但仍提及制裁，可能只是讨论
        return base
...
# 7) 大额转账/巨鲸 (patch v2.4 引入500万美金门槛，强化抛售意图识别)
...
def score_whale(text):
    ...
    usd = extract_usd_equiv(text)
    # 行为触发：无需金额阈值也可给较高分，但若金额极低则弱化
    if has_any(text, KW_WHALE_BEHAVIOUR):
        base = 0.55
        if usd > 5_000_000:
            base = clip01(0.55 + 0.20 * smooth_strength(usd, 5_000_000, 50_000_000))
        elif usd > 0:
            base = 0.45  # 少量但有行为
...
# 8) 行情异常波动 (patch v2.4 提高幅度要求，漏挽回调词已补)
...
# 12) 宏观 / 政策冲击风险 (patch v2.4 严格加密关联或强冲击，排除纯评论)
