...
# score_hack 新增：历史/过去漏洞语境减权
NEG_HACK_PAST = ["之前漏洞", "此前漏洞", "过去的攻击", "曾遭攻击", "之前被黑", "历史漏洞"]

# score_hack 新增：漏洞报告/安全研究类排除
...
# 1) 合约/链上漏洞攻击
KW_HACK = ["攻击", "被盗", "盗取", "重入", "闪电贷", "利用漏洞", "黑客", "exploit", "hacker", "入侵",
...
def score_hack(text):
    ...
    # 漏洞报告/安全研究排除
    if has_any(text, NEG_VULN_REPORT) and not has_any(text, ["损失", "被盗", "盗取", "资金消失", "资金损失"]):
        return 0.0
...
def score_volatility(text):
    ...
    if pct >= 50:
        ...
        return clip01(base)
    if pct >= 30:
        base = 0.30 if has_softener else 0.40
        if has_time:
            base += 0.05
        return clip01(base + 0.60 * smooth_strength(pct, 30, 50))
    if pct >= 15:
        base = 0.15 if has_softener else 0.25
        if has_time:
            base += 0.05
