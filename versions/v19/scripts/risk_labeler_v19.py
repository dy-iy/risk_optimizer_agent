...
def score_volatility(text):
    ...
    # 如果文本含有强信号词，忽略部分抑制
    has_strong_signal = has_any(text, KW_VOL_STRONG_SIGNAL + KW_VOL_MARKET_STRONG + ["暴跌", "闪崩", "瀑布", "腰斩", "插针"])
    ...
    if has_any(text, NEG_VOL_NON_MARKET) and not has_any(text, KW_SHOCK + KW_VOL_MISS + KW_VOL_MARKET_STRONG):
        return 0.0

    if has_any(text, NEG_VOL_NEUTRAL) and not has_any(text, KW_SHOCK + ["暴跌", "闪崩", "瀑布", "腰斩", "插针"] + KW_VOL_MARKET_STRONG):
        return 0.0
    ...
    if has_any(text, NEG_VOL_TECH) and not has_any(text, KW_SHOCK + ["暴跌", "闪崩"] + KW_VOL_MARKET_STRONG):
        return 0.0
    ...
    if has_any(text, NEG_POSITIVE_MOVE) and not has_strong_signal and not has_any(text, KW_VOL_MISS + KW_SHOCK + KW_VOL_MARKET_STRONG):
        return 0.0
