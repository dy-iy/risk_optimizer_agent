...
KW_HACK_BASE = ["攻击", "被盗", "盗取", "重入", "闪电贷", "利用漏洞", "黑客", "exploit", "hacker", "入侵",
                ...
                "坏账", "安全警钟", "社会工程攻击", "AI攻击", "热钱包被盗", "内部攻击", "凭证泄露", "社会工程攻击",
                "内部热钱包被盗"]
...
# 1) 合约/链上漏洞攻击 (patch v2.3 收紧触发条件，强化金额/实体证据)
def score_hack(text):
    # 商业无关新闻排除，除非存在明确攻击动作
    ...
    if has_any(text, KW_HACK):
        ...
        if usd > 0:
            ...
            if downweight:
                score = max(0.25, score * 0.35)  # 减权但不完全否定
            return score
        ...
        # 仅有攻击关键词但既无损失也无强攻击词，很可能只是背景提及，给低分
        base = 0.25
        if downweight:
            base = 0.05
        return base
...
# 3) 监管/法律风险 (patch v2.3 细化分级并扩展词库)
...
def score_regulatory(text):
    ...
    # 中性推动表述，且无强负面行动，极低分
    if has_any(text, NEG_REGULATORY_NEUTRAL) and not has_any(text, REG_STRONG_NEGATIVE):
        return 0.10

    # 强负面执法动作
    if has_any(text, REG_ACTORS) and has_any(text, REG_STRONG_NEGATIVE):
        return 0.80
...
# 7) 大额转账/巨鲸 (patch v2.3 扩充关键词，增加被动场景排除)
...
def score_whale(text):
    ...
    if has_any(text, KW_WHALE_BEHAVIOUR):
        base = 0.55
        if usd > 0:
            base = clip01(0.55 + 0.20 * smooth_strength(usd, 1_000_000, 50_000_000))
        return base * future_discount * position_discount

    # 弱信号不再独立处理，现在都作为强信号处理，但提高小额门槛到 1M
    if usd <= 1_000_000:
        return 0.15 * future_discount * position_discount
    return clip01(0.35 + 0.40 * smooth_strength(usd, 1_000_000, 100_000_000)) * future_discount * position_discount


# 8) 行情异常波动 (patch v2.3 扩充关键词并降低观点类压制)
KW_SHOCK = ["闪崩", "插针", "瀑布", "腰斩", "暴跌", "剧烈波动", "瞬间暴跌", "瞬间拉升"]


def score_volatility(text):
    # 排除访谈/观点/非市场内容，除非出现强波动信号或主要波动词
...
# 9) 项目治理 / 团队异常风险 (patch v2.3 补充关键词)
KW_TEAM = [
...
# 11) 基础设施 / 协议层异常风险 (patch v2.3 补充关键词)
KW_INFRA = [
...
# 12) 宏观 / 政策冲击风险 (patch v2.3 扩充关键词，强制加密市场关联或强冲击)
...
def score_macro(text):
    ...
    if not linked and not has_any(text, KW_MACRO_STRONG):
        return 0.0
