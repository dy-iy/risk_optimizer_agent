...
# 1) 合约/链上漏洞攻击 (patch v2.7 强化否定语境，过滤安全讨论)
def score_hack(text):
...
# 3) 监管/法律风险 (patch v2.7 收紧言论类得分上限)
...
def score_whale(text):
    ...
    if usd < 1_000_000:
        if usd < 500_000:
            # 没有任何金额，仍给最低分以便类别被选中
            return 0.12 * future_discount * position_discount
        return 0.12 * future_discount * position_discount
    if usd < 5_000_000:
        base = 0.15
        return clip01(base + 0.10 * smooth_strength(usd, 1_000_000, 5_000_000)) * future_discount * position_discount
    # 5M以上仍较高，但降低加成
    return clip01(0.25 + 0.30 * smooth_strength(usd, 5_000_000, 100_000_000)) * future_discount * position_discount


# 8) 行情异常波动 (patch v2.7 引入主流币种判断，非主流降权)
...
def score_volatility(text):
    ...
    if pct >= 50:
        ...
        if has_time:
            base += 0.05
        score = clip01(base)
        return max(score * major_factor, 0.12)  # 至少保证弱触发
    if pct >= 30:
...
# 10) 偿付能力 / 储备 / 流动性风险 (patch v2.7 扩充关键词，增加弱触发)
KW_SOLV = [
...
# 12) 宏观 / 政策冲击风险 (patch v2.7 收紧非冲击话语)
...
# 命中阈值：用于 rule_types
TYPE_THRESHOLD = 0.30
# 主类别选取最低阈值（v2.7 从0.20降至0.12，以减少漏标）
