...
def score_regulatory(text):
    # 放宽 early guard：仅命中文档无行动时给予低分
    if has_any(text, NEG_REG_ONLY_TALK) and not has_any(text, REG_ACTIONS):
...
def score_volatility(text):
    ...
    # 无负面波动词且无 shock 关键词，即使 pct>=8 也不触发
    if pct < 8:
        return 0.0
