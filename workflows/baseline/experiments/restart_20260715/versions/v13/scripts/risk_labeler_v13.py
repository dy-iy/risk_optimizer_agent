...
def score_hack(text):
    ...
    # 修改：恢复追回逻辑，但不完全压制强力攻击
    if has_any(text, NEG_HEALED_DEFENSE):
        if has_new_strong or has_severity_hack:
            # 不压制
            pass
        elif has_new_weak:
            return 0.35  # 适度降低
    ...
    if has_any(text, NEG_HACK_DISCUSSION) and not has_new_strong:
        if usd < 1_000_000:
            return 0.10
        else:
            return 0.20
...
def score_regulatory(text):
    # 新增：已解决执法降低
    if has_any(text, NEG_REG_RESOLVED) and not has_any(text, ACTIVE_PUNISH):
        # 已完成追回等，大幅降权
        base = 0.20
...
def score_liquidation(text):
    ...
    if has_any(text, KW_LIQ_EARLY):
        usd = extract_usd_equiv(text)
        base = 0.20 + 0.10 * smooth_strength(usd, 500_000, 2_000_000) if usd > 0 else 0.20  # 降
        if has_any(text, EVENT_CONFIRM) or has_any(text, KW_LIQ_STRONG):
            base = 0.40 + 0.20 * smooth_strength(usd, 500_000, 5_000_000)  # 降
        return clip01(base)
...
def score_whale(text):
    ...
    if not is_high_risk_action:
        # 即使有 KW_WHALE 或 action，但非高风险，极大降分
