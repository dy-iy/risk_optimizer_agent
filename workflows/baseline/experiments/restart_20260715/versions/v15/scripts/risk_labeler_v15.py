...
# 移除 NEG_HACK_DISCUSSION 中过于泛化的词，保留在函数中灵活使用
# 在函数内根据情况决定是否抑制
...
def score_hack(text):
    ...
    # 强愈合信号：大幅降权，但保留部分风险 (提升保底，避免真实攻击被过度压制)
    if has_heal:
    ...
    if has_any(text, NEG_HEALED_DEFENSE):
        if has_severity_hack:
            base = 0.50 if usd > 10_000_000 else 0.45
            return clip01(base)
        if has_new_strong:
            base = 0.40
            return clip01(base)
    ...
    if has_kw_hack:
        ...
        if has_new_weak:
            base += 0.08
        if has_severity_hack:
            base = max(base, 0.80)  # 高严重攻击起步 0.80
        # 金额boost
...
def score_regulatory(text):
    ...
    # 否定语境：若仅讨论/表示/批评且无执法实体，直接返回 0
    if has_any(text, NEG_REG_ONLY_TALK) and not has_any(text, ENFORCEMENT_ENTITIES):
        # 若确实同时有主动惩处，保留部分分数
    ...
    neg_talk_strong = has_any(text, NEG_REG_ONLY_TALK)  # 已包括 '表示' '称' 等
    ...
    # 特定词语境：若含有 '表示' '称' 且非实质性行动，强制降权（已在 early return 处理部分，此处保留）
    if has_any(text, ["表示", "称", "批评", "认为"]) and not (hard or active_pun or enforce):
        # 引用观点，非实际打击，大幅降权
        factor = 0.3
        if actor:
            factor = 0.4
        # 但在草案/提议中可能有风险，保持一定分数
        if draft_proposal:
            base = 0.35
            return clip01(base * factor)
        if hard:
            base = 0.55  # 但此处不会进，因为条件不满足
        else:
            base = 0.15
        if legal_positive:
            base *= 0.5
...
MASSIVE_WORDS = ["大规模"]
NEG_LIQ = ["合约地址", "智能合约", "智能合约内", "合约调用", "分析", "预警", "接近", "不可能"] + NEG_LIQ_CONTEXT  # 扩展讨论/预测词
...
def score_liquidation(text):
    ...
    if has_any(text, KW_LIQ_EARLY) or has_any(text, CONDITIONAL_WORDS):
        ...
        if has_any(text, EVENT_CONFIRM) or has_any(text, KW_LIQ_STRONG):
            ...
            # 无确认事件，但存在预警/条件词，给予 0.20 底线，而非 0.35
            base = 0.20
            if usd > 500_000:
    ...
    if has_any(text, KW_LIQ_CORE):
        ...
        if usd <= 0 and not has_event_confirm:
            ...
            base = max(0.20, 0.20 if usd < 100_000_000 else 0.25)
        else:
            base = 0.30
        if has_any(text, MARKET_NOISE):
            base *= 0.6
...
def score_solvency(text):
    ...
    has_misc = has_any(text, NEG_SOLV_MISC) and not has_strong  # 无关语境
...
# 提高弱 scorer 对应类型的优先级，以便在 close competition 中胜出
PRIMARY_PRIORITY = {
    "监管与法律风险": 10,
    "偿付能力 / 储备 / 流动性风险": 9,
    "链上漏洞 / 攻击风险": 8,
...
def score_all_risks(text: str) -> dict:
    ...
    # 诈骗 vs 漏洞 close competition (保留，增强)
