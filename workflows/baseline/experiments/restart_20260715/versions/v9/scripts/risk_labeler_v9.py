...
VOL_PREDICTION_WORDS = ["预警", "或将", "可能暴跌", "可能触发", "预计下跌", "若下跌"]  # narrowed
...
NEG_WHALE_MARKET_RECAP = ["行情", "播报", "24小时交易量", "数据", "排名"]
NEG_VOL_DISCUSSION = ["周期", "观点", "看法", "OTC"]
...
def score_hack(text):
    ...
    if has_any(text, NEG_HACK_DISCUSSION):
        if not has_new_strong:
            if usd < 1_000_000:
                ...
                return 0.30
        # has_new_strong 时继续正常流程

    # 新增：防御性/已修复语境，无新攻击迹象则极低分
    ...
    if has_any(text, NEG_NO_RISK):
        if has_kw_hack:
            return 0.20
    if has_kw_hack:
        base = 0.55 + 0.08 * smooth_strength(usd, 50_000, 200_000)
        # 严重性提升
        if usd > 10_000_000 or has_any(text, SEVERITY_HACK_BOOST):
            base += 0.15
        if is_discussion:
            base = 0.40 + 0.05 * smooth_strength(usd, 50_000, 200_000)
            if usd < 1_000_000:
                base *= 0.8
        if has_new_strong:
            base += 0.15
...
def score_regulatory(text):
    ...
    # 草案/提议，极低风险
    if draft_proposal:
        base = 0.25 if hard else (0.15 if soft and actor else (0.10 if actor else 0.05))
        if passive:
            base *= 0.8
        if neg_talk_strong:
            base *= 0.6
...
KW_LIQ_EARLY = ["期权到期", "资金费率", "强平价格", "爆仓额", "清算金额"]  # 移除 “清算强度”
...
NEG_WHALE_DISCUSSION = ["预测", "可能", "若", "如果", "评论", "历史"]  # 移除 “分析”“报道”
...
def score_whale(text):
    ...
    # 市场播报/交易量总结强抑制
    if has_any(text, NEG_WHALE_MARKET_RECAP) and (usd < 5_000_000 or not has_kw_whale):
        if usd < 1_000_000:
            return 0.0
        return 0.10
...
KW_SHOCK = ["闪崩", "插针", "瀑布", "腰斩", "暴跌", "暴涨", "剧烈波动", "瞬间暴跌", "瞬间拉升",
            "大涨", "飙升", "猛涨", "涨超", "跌超"]  # 添加 “跌超”
NEGATIVE_PRICE_MOVE = ["下跌", "跌幅", "下挫", "大幅下跌"]  # 移除 “暴跌”“跌超”“抛售”
...
def score_volatility(text):
    ...
    if pct >= 30 and has_surge:
        base = 0.45
        score = clip01(base + 0.30 * smooth_strength(pct, 30, 100))
        if is_analysis and not is_realtime:
            score *= 0.6
        if has_discussion:
            score *= 0.7
    ...
    if has_any(text, KW_SHOCK):
        if pct < 3:
            base = 0.35
            score = clip01(base + 0.10 * smooth_strength(max(pct, 1), 1, 5))
        else:
            base = 0.45
...
def score_macro(text):
    if has_any(text, KW_MACRO):
        ...
        if has_any(text, ["调查", "要求调查", "司法调查"]) and not has_any(text, ["军事冲突", "战争"]):
            pct = extract_max_pct(text)
            base = 0.10
            return clip01(base + 0.15 * smooth_strength(pct, 3, 10))
        pct = extract_max_pct(text)
        base = 0.35
