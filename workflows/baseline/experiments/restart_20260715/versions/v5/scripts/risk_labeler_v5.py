...
VOL_PREDICTION_WORDS = ["预测", "可能", "若", "如果", "预期", "分析", "讨论", "报道", "历史", "回顾"]
...
KW_HACK = ["漏洞", "攻击", "被盗", "盗取", "重入", "闪电贷", "利用漏洞", "黑客", "exploit", "hacker", "入侵",
           "铸造并抛售", "无故增发", "未授权铸造", "被利用", "窃取", "盗走", "安全事件"]
...
def score_hack(text):
    ...
    if has_heal:
        if has_new_strong:
            ...
            # 仅有弱新词且已修复，认为风险极低
            return 0.15 if has_kw_hack else 0.05
        else:
            # 已修复且无任何新攻击词
            if usd > 1_000_000:
                return 0.30
    ...
    if has_kw_hack:
        ...
        # 物理攻击词已在上方跳过，此处安全
        # 若匹配到弱新词但无 new_strong，适度提升
        if has_new_weak:
            base += 0.05
        return clip01(base)
    # 仅弱触发词（如“被利用”）无主攻击词，给低分
    if has_any(text, ["被利用", "窃取", "盗走", "安全事件"]):
        if usd > 100_000:
            return 0.25
        return 0.10
    return 0.0
...
ACTIVE_PUNISH = ["罚款", "逮捕", "起诉", "指控", "制裁", "拘留", "认罪"]

def score_regulatory(text):
    neg_talk = has_any(text, NEG_REG_ONLY_TALK)
...
NEG_LIQ = ["合约地址", "智能合约", "智能合约内", "合约调用", "分析", "预警", "接近", "可能"]
MARKET_NOISE = ["交易量", "清算量", "统计", "日均", "费用", "基金", "代币化"]
def score_liquidation(text):
    # 否定排除
    if has_any(text, NEG_LIQ):
        # 但是如果有强确认词（已爆仓等），仍然可能是真实事件，保留中等分数
        if has_any(text, EVENT_CONFIRM) or has_any(text, KW_LIQ_STRONG):
            usd = extract_usd_equiv(text)
            base = 0.40 + 0.30 * smooth_strength(usd, 500_000, 2_000_000) if usd > 0 else 0.40
            return clip01(base * 0.7)  # 有否定词降低
        return 0.0

    # 独立触发：爆仓/强平等强确认词
    if has_any(text, KW_LIQ_STRONG) or has_any(text, EVENT_CONFIRM):
        usd = extract_usd_equiv(text)
        base = 0.60 + 0.30 * smooth_strength(usd, 500_000, 2_000_000) if usd > 0 else 0.60
...
WHALE_ACTION = ["转入", "转出", "转移", "从未知地址", "未知地址", "大额异动", "链上大额",
...
def score_all_risks(text: str) -> dict:
    ...
    # 确定主类别
    # 若最高分低于 0.35，且差距小于 0.05，或金额无强支持，将 primary 设为无明显风险
