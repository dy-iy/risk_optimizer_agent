...
def score_hack(text):
    ...
    if has_heal:
        if has_new_strong:
            ...
            # 仅有弱新词且已修复，认为风险极低
            return 0.15 if has_any(text, KW_HACK) else 0.05
        else:
            # 已修复且无任何新攻击词
            if usd > 1_000_000:
                return 0.30
...
def score_regulatory(text):
    ...
    if has_any(text, REG_ACTORS) and has_any(text, HARD_ACTIONS):
        return 0.80
    # 调查相关逻辑
    if has_any(text, REG_ACTORS) and has_any(text, INVESTIGATION_WORDS):
        if has_any(text, ENFORCEMENT_ENTITIES):
            return 0.60
        else:
            return 0.25
    if has_any(text, REG_ACTORS) and has_any(text, SOFT_ACTIONS):
...
def score_liquidation(text):
    ...
    if has_any(text, KW_LIQ_CORE):
        ...
        if has_strong:
            usd = extract_usd_equiv(text)
            base = 0.55 + 0.30 * smooth_strength(usd, 500_000, 2_000_000) if usd > 0 else 0.55
        else:
            # 仅有“清算”一词，无强确认词
...
def score_all_risks(text: str) -> dict:
    ...
    # 确定主类别（考虑 close competition）
    primary_type_name = RISK_NAME_MAP[max_score_name] if max_score_01 >= 0.25 else "无明显风险"
    if max_score_01 >= 0.25 and (max_score_01 - second_score_01) < 0.05:
        # close competition: 检查 second 是否优先级更高
