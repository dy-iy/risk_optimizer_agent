...
KW_HACK = ["漏洞", "攻击", "被盗", "盗取", "重入", "闪电贷", "利用漏洞", "黑客", "exploit", "hacker", "入侵",
           "铸造并抛售", "无故增发", "未授权铸造"]
NEW_HACK_INDICATORS = ["零日", "新漏洞", "正在攻击", "未修复", "紧急", "最新攻击"]

def score_hack(text):
    # 已修复 / 历史事件 early guard
    if has_any(text, HEALED_REPORT) and not has_any(text, NEW_HACK_INDICATORS):
        return 0.05
    # 原否定逻辑仍保留，但利用相同的 guard 时覆盖
    if has_any(text, NEG_NO_RISK):
        if has_any(text, KW_HACK):
            return 0.20
    if has_any(text, KW_HACK):
        usd = extract_usd_equiv(text)
        return clip01(0.80 + 0.08 * smooth_strength(usd, 50_000, 200_000))
    return 0.0
...
# 硬执法行动
HARD_ACTIONS = ["起诉", "指控", "罚款", "制裁", "逮捕", "拘留", "认罪", "判决", "冻结", "查封", "禁令", "诉讼",
                "传唤", "调查", "和解"]
# 软性/非处罚行动
SOFT_ACTIONS = ["警告", "法案", "立法", "监管关注", "调查通知"]

def score_regulatory(text):
    if has_any(text, NEG_REG_ONLY_TALK):
        if not has_any(text, HARD_ACTIONS):
            # 无反面的软行动或仅角色
            if has_any(text, REG_ACTORS) and has_any(text, SOFT_ACTIONS):
                return 0.15
            if has_any(text, REG_ACTORS):
                return 0.10
            return 0.0
    if has_any(text, REG_ACTORS) and has_any(text, HARD_ACTIONS):
        return 0.80
    if has_any(text, REG_ACTORS) and has_any(text, SOFT_ACTIONS):
        return 0.45
    if has_any(text, REG_ACTORS):
...
# 6) 清算/爆仓
KW_LIQ_CORE = ["爆仓", "强平", "清算", "liquidation"]
KW_LIQ_EXTENDED = ["永续", "合约", "杠杆", "保证金"]
CONDITIONAL_WORDS = ["若", "如果", "可能触发", "可能清算", "可能爆仓", "假设", "可触发"]
EVENT_CONFIRM = ["实际清算", "大量爆仓", "大规模", "多头爆仓", "实际爆仓", "多头清算"]
NEG_LIQ = ["合约地址", "智能合约", "智能合约内", "合约调用"]
...
# 7) 大额转账/巨鲸
KW_WHALE = ["巨鲸", "whale", "转入", "转出", "转入交易所", "转出交易所", "从匿名地址", "从未知地址",
...
def score_whale(text):
    ...
    usd = extract_usd_equiv(text)
    if usd <= 0:
        return 0.35   # 提升至 TYPE_THRESHOLD 之上
...
def score_volatility(text):
    ...
    return clip01(base + 0.40 * smooth_strength(pct, 5, 20))
...
def score_all_risks(text: str) -> dict:
    ...
    # 主类别门槛提升至 0.25，避免极低分 scorer 抢占主类别
    result["rule_primary_type"] = RISK_NAME_MAP[max_score_name] if max_score_01 >= 0.25 else "无明显风险"
