...
# 1) 合约/链上漏洞攻击 (patch v3.0 强化否定语境，过滤安全讨论，并补充漏报关键词)
def score_hack(text):
    # 强烈否定词（安全运营、无任何损失）且无实际损失描述，直接返回0
    if has_any(text, NEG_HACK_STRONG_NO_RISK) and not has_any(text, ["损失", "被盗", "盗取", "资金丢失", "资金消失", "仍受损"]):
        return 0.0

    # v3 新增：安全产品/讨论/非攻击语境，若无攻击证据则直接归零
    if has_any(text, NEG_HACK_SAFETY):
        # 若仍出现明确攻击证据，允许继续，否则阻断
        if not has_any(text, ["攻击事件", "被黑", "盗取", "资金被盗", "遭受攻击", "利用漏洞", "黑客攻击", "漏洞攻击"]):
            return 0.0

    # 非攻击商业/金融行为直接排除（增持、购买等）
    ...
    if has_any(text, NEG_VULN_REPORT):
        if not has_any(text, ["损失", "被盗", "盗取", "资金消失", "资金损失", "事件", "攻击事件", "实际受损"]):
            return 0.0
    ...
    if has_any(text, KW_HACK):
        ...
        if has_attack_evid:
            ...
            if has_loss_evid:
                return max(0.6, 0.70)
            return 0.65 if has_loss_evid else 0.45
...
# 3) 监管/法律风险 (patch v3.0 严格区分讨论与执法，限制纯讨论评分)
...
def score_regulatory(text):
    ...
    # 中性推动表述，且无强负面行动 — 直接给极低分或抑制
    if has_any(text, NEG_REGULATORY_NEUTRAL) and not has_strong:
        score = 0.02 if discuss_discount else 0.05
        return clip01(score)

    # 强负面执法动作 — 但若有讨论折扣，限制上限0.4
    ...
    # 新增弱监管动作（申请不采取行动等）只在加密域且无强负面时给分
...
def score_liquidation(text):
    ...
    if has_any(text, KW_LIQ):
        ...
        if usd > 0:
            if usd < 500_000:
                score = clip01(0.35 + 0.15 * smooth_strength(usd, 100_000, 500_000))
            elif usd < 5_000_000:
                score = clip01(0.45 + 0.20 * smooth_strength(usd, 500_000, 5_000_000))
            else:
                # 最高限制至0.65
                score = clip01(0.50 + 0.30 * smooth_strength(usd, 5_000_000, 20_000_000))
...
# 7) 大额转账/巨鲸 (patch v3.0 提高非行为门槛，降低普通持仓误触发，并补充行为关键词)
...
def score_whale(text):
    ...
    # 非行为触发：提高金额门槛至 10M，低于该值极低分以避免主类别误选
    if usd < 10_000_000:
        return 0.05 * future_discount * position_discount
    # 10M 以上逐步提分
    return clip01(0.15 + 0.25 * smooth_strength(usd, 10_000_000, 100_000_000)) * future_discount * position_discount


# 8) 行情异常波动 (patch v3.0 重构触发，解决大量漏报并抑制日常行情)
...
def score_volatility(text):
    ...
    # 主流币种/市场整体性判断：若非主流，得分大幅折扣
    is_major = has_any(text, MAJOR_MARKET_TOKENS)
    major_factor = 1.0 if is_major else 0.4

    # 强波动信号（闪崩、恐慌抛售等）独立高分，无需百分比即可触发
    if has_any(text, KW_VOL_STRONG_SIGNAL):
        base = 0.55 if has_softener else 0.70
        if has_time:
            base += 0.05
        score = clip01(base + 0.15 * smooth_strength(pct, 10, 20))
        return score * major_factor
    ...
    if has_any(text, KW_SHOCK):
        ...
        if has_time:
            base += 0.05
        score = clip01(base + 0.20 * smooth_strength(pct, 15, 25))
        return score * major_factor
    ...
    if has_any(text, KW_VOL_MISS):
        ...
        if pct > 0 or has_time:
            ...
            # 无百分比也给予一个足够成为主类型的分值（0.25）
            base = 0.25 if not has_softener else 0.15
            score = base
        # 确保至少 0.12 但更高
        return max(score * major_factor, 0.12)
    ...
    if pct >= 30:
        ...
        if has_time:
            base += 0.05
        score = clip01(base + 0.50 * smooth_strength(pct, 30, 50))
        return max(score * major_factor, 0.12)
    if pct >= 25:
        base = 0.12 if has_softener else 0.18
        if has_time:
            base += 0.05
        score = clip01(base + 0.40 * smooth_strength(pct, 25, 45))
        return max(score * major_factor, 0.12)
    if pct >= 20:
        base = 0.08 if has_softener else 0.12
        if has_time:
            base += 0.05
        score = clip01(base + 0.30 * smooth_strength(pct, 20, 35))
        return max(score * major_factor, 0.12)
    if pct >= 15:
...
# 12) 宏观 / 政策冲击风险 (patch v3.0 收紧至真实事件，过滤纯观点/预测)
...
# 命中阈值：用于 rule_types
TYPE_THRESHOLD = 0.30
# 主类别选取最低阈值 (v3 保持 0.12)
PRIMARY_TYPE_MIN = 0.12
