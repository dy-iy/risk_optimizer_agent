...
# 高误报 scorer 集合（根据 analysis），用于主类别二次裁决
HIGH_FP_SCORERS = {'score_volatility', 'score_macro', 'score_regulatory', 'score_whale', 'score_hack'}
...
def score_all_risks(text: str) -> dict:
    ...
    # 主类别二次裁决：若最高分来自历史高误判 scorer 且分数未达阈值，则降级为无明显风险
    primary_type = RISK_NAME_MAP[max_score_name] if max_score_01 > 0 else "无明显风险"
    if max_score_name in HIGH_FP_SCORERS and max_score_01 < SECONDARY_THRESHOLD:
        # 再确认是否存在其他更高置信度的风险类别
        secondary_scores = {k: v for k, v in raw_scores.items() if k not in HIGH_FP_SCORERS}
        if secondary_scores:
            alt_max_name = max(secondary_scores, key=secondary_scores.get)
            alt_max_val = secondary_scores[alt_max_name]
            if alt_max_val > 0:
                # 若有其他类别得分也不低，维持原判
                if alt_max_val >= max_score_01 - 0.05:
                    pass
                else:
                    primary_type = "无明显风险"
            else:
    ...
    # 若 primary_type 变成无明显风险，风险分和主类别相应调整
    if primary_type == "无明显风险":
        effective_risk = 0
        effective_label = "low"
    else:
        effective_risk = int(round(max_score_01 * 100))
        effective_label = score_to_label(max_score_01)
