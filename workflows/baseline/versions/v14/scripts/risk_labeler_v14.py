...
# 1) 合约/链上漏洞攻击 (patch v2.6 重点修改：排除非攻击场景，补充漏报词，降低阈值)
def score_hack(text):
    ...
    if has_any(text, KW_HACK):
        ...
        if has_attack_evid:
            ...
            if has_loss_evid:
                return max(0.6, 0.70)
            # 有量子/漏洞/黑客冻结等强信号，保底0.35（确保超过0.2主类别阈值）
        ...
        if has_loss_evid:
            ...
            if mitigated:
                base = min(base, 0.4)
            return max(base, 0.35)  # 至少0.35

        # 仅有攻击关键词但既无损失也无强攻击词
        base = 0.30  # v2.5 为 0.25，提升以越过主类别阈值
        if downweight:
            base = 0.15
        if mitigated:
            base = min(base, 0.3)
...
# 3) 监管/法律风险 (patch v2.6 收紧触发，增加弱信号)
...
# 6) 清算/爆仓 (patch v2.6 按金额分级，排除非加密清算)
...
def score_liquidation(text):
    ...
    if has_any(text, KW_LIQ):
        ...
        if usd > 0:
            # 按金额分级，降低小规模评分
            if usd < 500_000:
                return clip01(0.35 + 0.15 * smooth_strength(usd, 100_000, 500_000))
            elif usd < 5_000_000:
                return clip01(0.45 + 0.20 * smooth_strength(usd, 500_000, 5_000_000))
            else:
                return clip01(0.55 + 0.30 * smooth_strength(usd, 5_000_000, 20_000_000))
        if has_strong_liq:
            return 0.55  # 降为0.55（原0.65）
        return 0.40  # 降为0.40（原0.50）
    if has_any(text, KW_LIQ_WEAK):
        return 0.12
    return 0.0


# 7) 大额转账/巨鲸 (patch v2.6 降低基础分，增加风险上下文，补充召回词)
...
def score_whale(text):
    ...
    # 非行为触发：金额门槛降至1M，但基础分降低
    if usd < 1_000_000:
        if usd < 500_000:
            return 0.0
        return 0.08 * future_discount * position_discount
    if usd < 5_000_000:
        base = 0.15  # 原0.20
...
# 8) 行情异常波动 (patch v2.6 提高百分比门槛，排除技术术语，增加弱信号)
...
def score_volatility(text):
    ...
    if pct >= 25:
        ...
        if has_time:
            base += 0.05
        return clip01(base + 0.40 * smooth_strength(pct, 25, 45))
    if pct >= 20:
        base = 0.08 if has_softener else 0.12
        if has_time:
            base += 0.05
        return clip01(base + 0.30 * smooth_strength(pct, 20, 35))
    if pct >= 15:
...
# 12) 宏观 / 政策冲击风险 (patch v2.6 增加利好方向抑制)
...
# 命中阈值：用于 rule_types
TYPE_THRESHOLD = 0.30
# 主类别选取最低阈值（v2.6 从0.30降至0.20，以提高召回率）
PRIMARY_TYPE_MIN = 0.20
