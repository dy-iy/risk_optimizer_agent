...
OUT_PATH = os.environ.get("OUT_PATH", r".\versions\v20\reports\predictions\risk_labeler_v20_output.csv")
...
NEG_REG_ONLY_TALK = ["呼吁", "建议", "提议", "敦促", "讨论", "区分", "澄清", "拟", "征求意见"]
...
# 新增波动加密域入口：主要代币/市场词，避免仅因 CRYPTO_DOMAIN 缺失而完全 0 分
VOL_CRYPTO_ENTRY = CRYPTO_DOMAIN + ["BTC", "比特币", "ETH", "以太坊", "XRP", "ADA", "SOL", "BNB", "USDT", "USDC", "币价", "行情"]
...
NEG_HACK_RESEARCH = ["研究", "报告", "识别", "计划", "事后分析", "事后", "回顾", "学术", "论文", "安全研究", "漏洞研究", "漏洞报告", "漏洞披露", "漏洞挖掘", "渗透测试", "漏洞复现", "漏洞分析", "安全审计"]
...
NEG_HACK_SAFETY = ["安全产品", "安全服务", "安全合作", "安全响应", "安全提升", "安全公告", "安全运营", "安全无虞",
                   ...
                   "AI安全", "AI 安全", "安全AI", "AI防御", "安全趋势", "安全峰会", "安全领域", "安全赛道", "安全生态", "安全月"]
...
# 高严重性真实攻击指示
KW_HACK_HIGH_SEV = ["被盗", "窃取", "资金被盗", "黑客盗取", "攻击成功", "损失金额", "实际损失", "资金损失", "被害", "被盗资金", "密钥恢复", "快速恢复", "攻击合约", "利用漏洞攻击", "安全事件爆发", "紧急修复", "冻结黑客地址", "截留资金", "特权地址结留"]
...
NEG_WHALE_FALSE = ["融资", "市值", "交易量", "牌照", "总市值", "流通市值", "成交额", "交易额", "成交量",
                   ...
                   "赔偿", "退款", "受害者", "受害方",
                   "股票回购", "股份回购", "公司购回", "股票", "股权", "除牌"]

NEG_WHALE_PRODUCT_DESC = ["推出", "上线", "平台", "协议", "DeFi", "借贷", "流动性池", "AMM", "质押", "挖矿", "产品"]

# 强鲸鱼行为信号，可跳过 NEG_WHALE_FALSE 的严格排除
STRONG_WHALE_SIGNAL = ["鲸鱼", "巨鲸", "大户", "大型持有者", "链上异动", "大额转移", "巨额转账"]
...
REG_LEGISLATIVE_SIGNALS = ["提案", "草案", "法案", "立法", "公众咨询", "咨询期"]
...
# 新增的强市场波动词汇，用于激活 scorer
KW_VOL_MARKET_STRONG = ["暴跌", "暴涨", "跳水", "急跌", "急拉", "暴拉", "崩跌", "恐慌下跌", "恐慌大涨", "价格崩溃", "剧烈波动", "剧烈震荡", "价崩", "断崖式下跌", "单日暴跌", "单日暴涨"]
...
NEG_MACRO_NON_IMPACT = ["会议", "峰会", "论坛", "研讨会", "讨论会", "圆桌", "沙龙", "播客讨论"]
...
NEG_OUTAGE_NON_EXCHANGE = ["论坛", "讨论", "开源", "项目维护回应"]
...
def score_hack(text):
    ...
    # 研究/报告/回顾语境，无真实损失立刻归零
    if has_any(text, NEG_HACK_RESEARCH) and not has_any(text, ["损失", "被盗", "资金被盗", "漏洞攻击", "利用漏洞", "黑客攻击", "攻击事件"]):
        return 0.0
    ...
    # 非攻击商业/金融行为直接排除，但放宽豁免词至包含"黑客"
    if has_any(text, NEG_HACK_NON_ATTACK) and not has_any(text, ["漏洞", "攻击", "被黑", "安全事件", "黑客"]):
        return 0.0

    if has_any(text, NEG_HACK_BUSINESS_EXCLUDE) and not has_any(text, ["漏洞", "攻击", "被黑", "安全事件", "黑客"]):
        return 0.0
    ...
    high_sev = has_any(text, KW_HACK_HIGH_SEV)  # 严重性指示
    ...
    if has_any(text, KW_HACK):
        ...
        if has_any(text, KW_VULN_HIGH):
            ...
            if has_real_loss:
                score_val += 0.10
            if high_sev:
                score_val = max(score_val, 0.7)
            if mitigated:
                score_val = min(score_val, 0.40 if has_attack_evid else 0.35)
            return min(score_val, upper_bound)

        # 事后处理关键词
        if has_any(text, KW_HACK_POST_EVENT):
            score_val = 0.35
            if has_real_loss or usd > 0 or high_sev:
                score_val = 0.5
            if mitigated:
                score_val = min(score_val, 0.40 if has_attack_evid else 0.30)
            return min(score_val, upper_bound)

        # 防御语境
        if mitigated and not has_loss_evid and not high_sev:
        ...
        if usd > 0:
            if high_sev:
                ...
                if mitigated:
                    score_val = min(score_val, 0.40 if has_attack_evid else 0.35)
                score_val = max(score_val, 0.5)
            else:
                base = 0.45
                score_val = clip01(base + 0.05 * smooth_strength(usd, 1_000_000, 30_000_000))
                if downweight:
                    score_val = max(0.25, score_val * 0.55)
                if mitigated:
                    score_val = min(score_val, 0.35 if has_attack_evid else 0.30)
                score_val = min(score_val, 0.45)
            return min(score_val, upper_bound)
        ...
        if has_attack_evid:
            if high_sev:
                ...
                if downweight and not has_loss_evid:
                    ...
                    base = 0.30
            if mitigated:
                base = min(base, 0.40 if has_attack_evid else 0.30)
            return min(base, upper_bound)

        # 有损失描述但无攻击证据
        if has_loss_evid:
            base = 0.50 if high_sev else 0.40
            if downweight:
                base = 0.35
            if mitigated:
                base = min(base, 0.40 if has_attack_evid else 0.35)
        ...
        if mitigated:
            base = min(base, 0.20)
        return min(base if not high_sev else 0.3, upper_bound)
    return 0.0
...
def score_fraud(text):
    ...
    if has_any(text, KW_FRAUD):
        # 强严重词
        severe_fraud = has_any(text, ["诈骗", "庞氏", "传销", "跑路", "rug", "rugpull"])
        if severe_fraud:
            base = 0.45
        else:
            base = 0.25
...
def score_regulatory(text):
    ...
    # 修改 NEG_REG_ONLY_TALK：仅在无强负面且无实质立法/提案信号时归零
    only_talk = has_any(text, NEG_REG_ONLY_TALK)
    has_formal_signal = has_any(text, REG_LEGISLATIVE_SIGNALS) or has_any(text, ["获得批准", "已获批", "监管部门批准", "监管放行"])
    if only_talk and not has_any(text, REG_STRONG_NEGATIVE) and not has_formal_signal:
        return 0.0
    ...
    # 提升立法类弱信号得分
    if has_crypto and has_any(text, REG_LEGISLATIVE_SIGNALS):
        base = 0.22 if not discuss_discount else 0.15
        return clip01(base)
...
def score_outage(text):
    ...
    # 非交易所语境降分
    if has_any(text, NEG_OUTAGE_NON_EXCHANGE):
        return 0.10
...
KW_LIQ_WEAK = ["期权集中到期", "负资金费率", "空头挤压", "逼空", "清算风险", "爆仓风险", "强平风险", "清算压力", "清算事件", "爆仓规模"]
KW_LIQ_STRONG = ["清算强度", "清算金额", "将清算", "发生清算", "已清算", "实际清算"]


def score_liquidation(text):
    if has_any(text, ["ATM 交易量", "ATM交易量"]) or (has_any(text, ["清算了约", "清算交易量"]) and "交易量" in text):
        return 0.0
    ...
    if has_any(text, NEG_LIQ_FALSE):
        if not has_strong_liq and not extract_usd_equiv(text) > 0:
            return 0.0
...
def score_whale(text):
    # 强鲸鱼信号时，跳过 NEG_WHALE_FALSE 严格排除
    has_strong_whale_signal = has_any(text, STRONG_WHALE_SIGNAL + KW_WHALE_BEHAVIOUR)
    if not has_strong_whale_signal and has_any(text, NEG_WHALE_FALSE):
        return 0.0
    ...
    # 必须至少包含链上地址/哈希/钱包证据或大额金额，否则大概率只是普通交易描述
    has_onchain_evidence = has_any(text, ["地址", "钱包", "链上", "交易哈希", "txid", "TXID", "哈希"])
    usd = extract_usd_equiv(text)
    if not has_onchain_evidence and usd == 0:
        return 0.0
...
# 定义所有强波动词，用于豁免 NEG 守卫
STRONG_VOL_EXEMPT = KW_VOL_STRONG_SIGNAL + KW_VOL_MARKET_STRONG + KW_SHOCK + KW_VOL_MISS


def score_volatility(text):
    # 放宽加密域入口
    if not (has_any(text, CRYPTO_DOMAIN) or has_any(text, ["BTC", "比特币", "ETH", "以太坊", "币价", "行情"])):
        return 0.0

    has_strong_signal = has_any(text, STRONG_VOL_EXEMPT)
    ...
    # 仅当无强信号时才让以下守卫完全归零
    if has_any(text, NEG_VOL_NON_MARKET) and not has_strong_signal:
        return 0.0

    if has_any(text, NEG_VOL_NEUTRAL) and not has_strong_signal:
        return 0.0
    ...
    if has_any(text, NEG_VOL_TECH) and not has_strong_signal:
        return 0.0
    ...
    if has_any(text, NEG_POSITIVE_MOVE) and not has_strong_signal:
        return 0.0
    ...
    # 普通强波动关键词（含新增）
    if has_any(text, KW_SHOCK + KW_VOL_MARKET_STRONG):
    ...
    if has_any(text, KW_VOL_MISS):
        base = 0.22 if has_softener else 0.30
        if has_time:
            base += 0.05
        if pct > 0 or has_time:
            ...
            score_val = base
        return max(score_val * major_factor * daily_discount, 0.06)

    # 仅百分比触发
    if pct >= 50:
        base = 0.25 if has_softener else 0.30
        if has_time:
            base += 0.05
        score_val = clip01(base)
        return max(score_val * major_factor * daily_discount, 0.06)
    if pct >= 30:
        base = 0.15 if has_softener else 0.20
        if has_time:
            base += 0.05
        score_val = clip01(base + 0.30 * smooth_strength(pct, 30, 50))
        return max(score_val * major_factor * daily_discount, 0.06)
    if pct >= 25:
        base = 0.10 if has_softener else 0.14
        if has_time:
            base += 0.05
        score_val = clip01(base + 0.25 * smooth_strength(pct, 25, 45))
        return max(score_val * major_factor * daily_discount, 0.04)
    if pct >= 20:
        base = 0.06 if has_softener else 0.08
        if has_time:
            base += 0.05
        score_val = clip01(base + 0.20 * smooth_strength(pct, 20, 35))
        return max(score_val * major_factor * daily_discount, 0.04)
    if pct >= 15:
        base = 0.04 if has_softener else 0.06
        if has_time:
            base += 0.05
...
def score_macro(text):
    ...
    if has_any(text, NEG_MACRO_COMMENT) and not has_any(text, KW_MACRO_STRONG):
        return 0.0
    # 非冲击会议/讨论直接排除
    if has_any(text, NEG_MACRO_NON_IMPACT) and not has_any(text, KW_MACRO_STRONG):
        return 0.0
    ...
    if has_any(text, NEG_MACRO_COMMENT) and not strong:
        return 0.0
    if has_any(text, NEG_MACRO_NON_IMPACT) and not strong:
        return 0.0
...
TYPE_THRESHOLD = 0.30
PRIMARY_TYPE_MIN = 0.40  # 与 low/medium 风险等级边界对齐
PRIMARY_MARGIN_REQUIRED = 0.08  # 弱区间必须领先第二名的最小差距
...
def score_all_risks(text: str) -> dict:
    ...
    if max_val >= PRIMARY_TYPE_MIN:
        # 弱信号区间需要领先 margin，否则退回无明显风险
        if max_val < 0.25 and (max_val - second_val) < PRIMARY_MARGIN_REQUIRED:
            primary_type = "无明显风险"
        else:
