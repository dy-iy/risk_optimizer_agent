...
# 新增：正面行情缓和词
NEG_POSITIVE_MOVE = ["上涨", "拉升", "反弹", "回升", "走高", "突破", "创新高", "大涨", "飙升", "暴涨"]
...
def score_whale(text):
    ...
    if has_any(text, KW_WHALE_WEAK) and not has_any(text, KW_WHALE):
        if usd <= 0:
            return 0.15
        return clip01(0.25 + 0.20 * smooth_strength(usd, 200_000, 1_500_000))
...
def score_volatility(text):
    ...
    if has_any(text, NEG_POSITIVE_MOVE) and not has_any(text, ["暴跌", "闪崩", "瀑布", "腰斩"]):
        return 0.0
    ...
    return clip01(base + 0.40 * smooth_strength(pct, 15, 30))
