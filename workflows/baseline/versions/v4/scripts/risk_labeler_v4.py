...
NEG_POSITIVE_MOVE = ["小幅上涨", "微涨", "温和上涨", "企稳回升", "慢牛", "持续走高"]  # 修改：只保留温和上涨
NEG_STAT_DESC = ["平均", "均值", "中位数", "标准差", "方差", "统计", "概率", "分布", "回归", "相关性"]

# 新增：score_hack 链上/加密领域限定词
CRYPTO_DOMAIN = ["链上", "智能合约", "DeFi", "代币", "token", "Token", "TOKEN", "合约地址", "私钥", "钱包", "跨链", "预言机", "DApp", "dApp", "NFT", "nft", "矿工", "区块", "交易哈希", "gas", "GAS", "Gas"]

# 新增：score_hack 非加密领域否定词
NEG_NON_CRYPTO_HACK = ["医疗机构", "医疗数据", "AI", "人工智能", "软件", "企业网络", "网络安全", "数据泄露", "黑客松", "hackathon", "Hackathon"]

# 新增：score_hack 否定语境（漏洞报告/已修复等）
NEG_HACK_FALSE = ["追回", "已修复", "完成修复", "报告漏洞", "漏洞赏金", "不受影响", "未受影响", "安然无恙", "无资产损失", "未造成损失", "无资金损失", "已经恢复", "误报"]

# 新增：score_liquidation 否定词
...
def score_hack(text):
    if has_any(text, NEG_HACK_FALSE) and not has_any(text, ["仍受损", "未被追回", "资金消失"]):  # 已修复/报告但仍有损失除外
        if has_any(text, KW_HACK):
            return 0.20
    if has_any(text, NEG_NON_CRYPTO_HACK):
        return 0.0
    if has_any(text, KW_HACK):
        if has_any(text, CRYPTO_DOMAIN):
...
def score_regulatory(text):
    ...
    if has_any(text, ["SEC"]) and has_any(text, NEG_REG_ONLY_TALK) and not has_any(text, REG_ACTIONS):
        return 0.0
    # 正面/中性监管新闻降级
    if has_any(text, NEG_REGULATORY_POSITIVE) and not has_any(text, REG_ACTIONS):
        return 0.15
...
def score_whale(text):
    ...
    if has_any(text, KW_WHALE_WEAK) and not has_any(text, KW_WHALE):
        if usd <= 1_000_000:
            return 0.05
        return clip01(0.25 + 0.20 * smooth_strength(usd, 50_000_000, 200_000_000))
    
    # strong signals
    if usd <= 500_000:
        return 0.25
    return clip01(0.35 + 0.40 * smooth_strength(usd, 10_000_000, 100_000_000))


# 8) 行情异常波动
KW_SHOCK = ["闪崩", "插针", "瀑布", "腰斩", "暴跌", "暴涨", "剧烈波动", "瞬间暴跌", "瞬间拉升",
            "大涨", "飙升", "狂飙", "价格飞涨", "暴涨暴跌"]  # 加入正面剧烈词
def score_volatility(text):
    ...
    if pct >= 10:
        ...
        if has_time:
            base += 0.05
        return clip01(base + 0.30 * smooth_strength(pct, 10, 30))
