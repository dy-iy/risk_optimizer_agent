...
ANALYZER_OUTPUT_SCHEMA = {
    ...
    "patch_contract": {
        "focus_metric": "false_positive_rows / false_negative_rows / type_mismatch_rows",
        "single_hypothesis": "one falsifiable mechanism to change in this iteration",
        "target": "one scorer or one arbitration function",
        "allowed_code_regions": ["function_or_constant_name"],
        "target_news_ids": ["rows expected to improve"],
        "must_preserve_news_ids": ["contrastive control rows that must not regress"],
        "forbidden_transitions": ["false_positive->type_mismatch", "correct->type_mismatch"],
        "expected_delta": {
            "false_positive": "negative / unchanged",
            "false_negative": "unchanged",
            "type_mismatch": "unchanged"
        },
...
ANALYZER_TASK_INSTRUCTIONS = [
    "Before proposing a patch, inspect transition_risk_analysis. Treat vulnerable error conversions and must-preserve rows as hard contrastive controls, not as ordinary examples.",
    "Return exactly one structured patch_contract. It must name one hypothesis, one target mechanism, target row IDs, preserve row IDs, expected metric deltas, and forbidden error transitions.",
    "A proposal is unsafe when it merely converts the focus error into another target error. Explicitly predict FP->TM, FP->FN, FN->TM, and correct->error migrations whenever the decision dependencies make them possible.",
    ...
    "如果 version_history.regression_summary.plateau_metrics 非空，必须只选择一个停滞指标作为下一轮 focus_metric，并给出可由错误样本直接证伪的单一修复假设。",
    "如果 slice_diagnostics.primary_policy_counterfactual 显示只改主类别仲裁即可显著降低 type_mismatch，优先修改仲裁策略，不要扩大或收紧所有 scorer。",
    "把 type_mismatch_kind 区分为 overassigned_weak_type、missed_primary_type、cross_type_confusion；三类根因和修法不同，不得混为一个关键词问题。",
    "下一轮 patch_plan 原则上只修改一个独立机制（一个 scorer 或主类别仲裁），形成可归因的 coordinate-descent 实验；不要把多个互不相关的改动塞进同一候选。",
