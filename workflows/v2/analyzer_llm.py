...
from tools.paths import resolve_project_root, resolve_versions_root
...
def extract_source_context(source_script: str | Path | None) -> dict:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.If):
            continue
        condition = ast.get_source_segment(source_code, node.test) or ""
        body_source = "\n".join(
            ast.get_source_segment(source_code, child) or "" for child in node.body
        )
        if (
            "rule_label" in condition
            and "low" in condition.lower()
            and "rule_primary_type" in body_source
        ):
            decision_dependencies.append(
                {
                    "upstream": "rule_risk_score -> rule_label",
                    "downstream": "rule_primary_type",
                    "condition": clip_source(condition, limit=260),
                    "effect": clip_source(body_source, limit=360),
                    "risk": (
                        "A score change that crosses the low-label threshold can also "
    ...
    return {
        ...
        "decision_dependencies": decision_dependencies[:12],
    }
...
def build_regression_summary(metrics: list[dict]) -> dict:
    ...
    plateau_metrics = []
    for key in ["false_positive_rows", "false_negative_rows", "type_mismatch_rows"]:
        history = [
            safe_float(item.get(key))
            for item in metrics[-6:]
            if isinstance(item.get(key), (int, float))
        ]
        if len(history) < 3:
            continue
        prior_best = min(history[:-1])
        current_value = history[-1]
        if current_value >= prior_best:
            plateau_metrics.append(
                {
                    "metric": key,
                    "window_values": [round_float(value) for value in history],
                    "current": round_float(current_value),
                    "prior_best": round_float(prior_best),
                    "reason": "最近一版没有刷新历史窗口最优值",
                }
            )
    ...
    if plateau_metrics:
        notes.append(
            "检测到停滞指标: "
            + ", ".join(item["metric"] for item in plateau_metrics)
            + "；下一候选应采用定向修复并要求该指标实际下降。"
        )
...
def _dominant_scorer_summary(df: pd.DataFrame, topn: int = 12) -> list[dict]:
    score_cols = [column for column in SCORE_COLS if column in df.columns]
    if df.empty or not score_cols:
        return []
    numeric = df[score_cols].apply(lambda column: column.map(safe_float))
    dominant = numeric.idxmax(axis=1)
    dominant_value = numeric.max(axis=1)
    counts = (
        pd.DataFrame({"score_col": dominant, "value": dominant_value})
        .groupby("score_col")
        .agg(count=("score_col", "size"), avg_value=("value", "mean"))
        .reset_index()
        .sort_values("count", ascending=False)
        .head(topn)
    )
    counts["avg_value"] = counts["avg_value"].round(4)
    return counts.to_dict(orient="records")


def build_transition_risk_analysis(
    merged_df: pd.DataFrame,
    source_context: Optional[dict] = None,
    sample_rows: int = DEFAULT_SAMPLE_ROWS_PER_BUCKET,
    text_limit: int = DEFAULT_TEXT_LIMIT,
) -> dict:
    """Expose rows likely to migrate between error buckets after a score patch."""
    source_context = source_context or {}
    required = {
        "gold_risk_label",
        "rule_risk_label",
    ...
    if merged_df.empty or not required.issubset(merged_df.columns):
        return {
            "available": False,
            "reason": "merged data is unavailable or missing required columns",
            "decision_dependencies": source_context.get("decision_dependencies", []),
        }

    work = merged_df.copy()
    gold_label = normalize_str_col(work, "gold_risk_label").str.lower()
    rule_label = normalize_str_col(work, "rule_risk_label").str.lower()
    gold_type = normalize_str_col(work, "gold_primary_risk_type")
    rule_type = normalize_str_col(work, "rule_primary_risk_type")
    same_type = gold_type == rule_type

    # These rows look like false positives today, but lowering the severity score
    # can make the label correct while a coupled low-label policy erases the type.
    vulnerable_mask = (
        (gold_label == "low")
        & rule_label.isin(["medium", "high"])
        & same_type
        & gold_type.ne("")
    )
    vulnerable = work.loc[vulnerable_mask].copy()

    gold_types_text = normalize_str_col(work, "gold_risk_types") if "gold_risk_types" in work.columns else pd.Series("", index=work.index)
    has_typed_gold = ~gold_types_text.str.lower().isin(["", "[]", "nan", "none"])
    correct_low_typed_mask = (
        (gold_label == "low")
        & (rule_label == "low")
        & same_type
        & has_typed_gold
    )
    correct_low_typed = work.loc[correct_low_typed_mask].copy()

    rule_scores = (
        work["rule_risk_score"].map(safe_float)
        if "rule_risk_score" in work.columns
        else pd.Series(0.0, index=work.index)
    )
    threshold_mask = rule_scores.between(30.0, 45.0, inclusive="both") | rule_scores.between(
        65.0, 75.0, inclusive="both"
    )
    near_threshold = work.loc[threshold_mask].copy()

    vulnerable_cases = build_representative_cases(
        vulnerable,
        "false_positive",
    ...
    preserve_cases = build_representative_cases(
        correct_low_typed,
        "type_mismatch",
        max_rows=sample_rows,
        text_limit=text_limit,
    ...
    threshold_cases = build_representative_cases(
        near_threshold,
        "score_diff",
        max_rows=sample_rows,
        text_limit=text_limit,
    ...
    return {
        "available": True,
        "decision_dependencies": source_context.get("decision_dependencies", []),
        "vulnerable_error_conversions": {
            "false_positive_to_type_mismatch_if_label_is_lowered": int(len(vulnerable)),
            "by_primary_type": value_counts_top(gold_type.loc[vulnerable_mask], topn=12),
            "by_dominant_scorer": _dominant_scorer_summary(vulnerable),
            "cases": vulnerable_cases,
        },
        "must_preserve_sets": {
            "correct_low_with_typed_primary_rows": int(len(correct_low_typed)),
            "correct_low_with_typed_primary_cases": preserve_cases,
            "near_label_threshold_rows": int(len(near_threshold)),
            "near_label_threshold_cases": threshold_cases,
        },
        "required_impact_report": {
            "transition_matrix": True,
            "changed_rows": True,
            "target_metric_deltas": True,
        },
    }


def build_patch_contract(payload: dict, llm_analysis: dict) -> dict:
    """Normalize the LLM proposal into an enforceable, fail-closed contract."""
    proposed = llm_analysis.get("patch_contract", {})
    if not isinstance(proposed, dict):
        proposed = {}
    focus_metric = str(
        proposed.get("focus_metric")
        or payload.get("optimization_contract", {}).get("focus_metric", "")
    )
    focus_bucket = {
        "false_positive_rows": "false_positive",
        "false_negative_rows": "false_negative",
        "type_mismatch_rows": "type_mismatch",
    }.get(focus_metric, "")
    target_cases = payload.get("sample_cases", {}).get(focus_bucket, []) if focus_bucket else []
    target_ids = proposed.get("target_news_ids", [])
    if not isinstance(target_ids, list) or not target_ids:
        target_ids = [str(item.get("news_id", "")) for item in target_cases if item.get("news_id")]

    transition = payload.get("transition_risk_analysis", {})
    vulnerable = transition.get("vulnerable_error_conversions", {}).get("cases", [])
    preserved = transition.get("must_preserve_sets", {}).get(
        "correct_low_with_typed_primary_cases", []
    )
    default_preserve_ids = [
        str(item.get("news_id", ""))
        for item in list(vulnerable) + list(preserved)
        if item.get("news_id")
    ]
    preserve_ids = proposed.get("must_preserve_news_ids", [])
    if not isinstance(preserve_ids, list) or not preserve_ids:
        preserve_ids = default_preserve_ids

    patch_plan = llm_analysis.get("patch_plan", [])
    first_plan = patch_plan[0] if isinstance(patch_plan, list) and patch_plan else {}
    if not isinstance(first_plan, dict):
        first_plan = {}
    allowed_regions = proposed.get("allowed_code_regions", [])
    if not isinstance(allowed_regions, list):
        allowed_regions = []
    target = str(proposed.get("target") or first_plan.get("target") or "")
    if not allowed_regions and target:
        allowed_regions = [target]

    forbidden = proposed.get("forbidden_transitions", [])
    ...
    return {
        "contract_version": "patch_contract_v1",
        "focus_metric": focus_metric,
        "single_hypothesis": str(
            proposed.get("single_hypothesis")
            or first_plan.get("action")
            or ""
        ),
        "target": target,
        "allowed_code_regions": allowed_regions,
        "target_news_ids": list(dict.fromkeys(str(value) for value in target_ids if str(value))),
        "must_preserve_news_ids": list(
            dict.fromkeys(str(value) for value in preserve_ids if str(value))
        ),
        ...
        "expected_delta": proposed.get("expected_delta", {}),
        "acceptance_criteria": {
            "focus_metric_must_improve": True,
            "no_target_metric_may_regress": True,
            "changed_row_preflight_must_pass": True,
        },
        "confidence": str(proposed.get("confidence") or llm_analysis.get("confidence", "")),
    }
...
def build_analysis_payload(
    ...
    source_context: Optional[dict] = None,
    slice_diagnostics: Optional[dict] = None,
    focus_metric: str = "",
    ...
    payload = {
        ...
        "statistics": {
            ...
            "type_mismatch_analysis": {
                "mismatch_kind_top": value_counts_top(
                    normalize_str_col(tm_df, "type_mismatch_kind"), topn=10
                ),
                "risk_label_top": value_counts_top(
                    normalize_str_col(tm_df, "gold_risk_label"), topn=5
                ),
    ...
    payload["transition_risk_analysis"] = build_transition_risk_analysis(
        merged_df if merged_df is not None else pd.DataFrame(),
        source_context=source_context,
        sample_rows=sample_rows,
        text_limit=text_limit,
    )
    if slice_diagnostics:
        payload["slice_diagnostics"] = slice_diagnostics
    payload["optimization_contract"] = {
        "focus_metric": focus_metric,
        "promotion_policy": "constrained_pareto_v1",
        "hard_constraints": [
            "false_positive_rows must not regress",
            "false_negative_rows must not regress",
            "type_mismatch_rows must not regress",
        ],
        "promotion_requirement": "focus_metric must improve and no hard constraint may regress",
    }
    if version_metrics:
...
def build_full_analysis(
    ...
    full_analysis = {
        ...
        "sample_cases": payload.get("sample_cases", {}),
        "transition_risk_analysis": payload.get("transition_risk_analysis", {}),
        "llm_analysis": llm_analysis,
        "patch_contract": build_patch_contract(payload, llm_analysis),
        "meta": {
            "model": model,
    ...
    if "slice_diagnostics" in payload:
        full_analysis["slice_diagnostics"] = payload["slice_diagnostics"]
    if "optimization_contract" in payload:
        full_analysis["optimization_contract"] = payload["optimization_contract"]
...
def render_markdown_report(full_analysis: dict) -> str:
    ...
    transition = full_analysis.get("transition_risk_analysis", {}) or {}
    vulnerable = transition.get("vulnerable_error_conversions", {}) or {}
    preserve = transition.get("must_preserve_sets", {}) or {}
    if transition.get("available"):
        lines.append("## Transition-risk controls")
        lines.append("")
        lines.append(
            "- Potential false_positive -> type_mismatch rows: "
            + str(vulnerable.get("false_positive_to_type_mismatch_if_label_is_lowered", 0))
        )
        lines.append(
            "- Correct low-label typed controls: "
            + str(preserve.get("correct_low_with_typed_primary_rows", 0))
        )
        lines.append(
            "- Near-threshold controls: "
            + str(preserve.get("near_label_threshold_rows", 0))
        )
        lines.append("")

    contract = full_analysis.get("patch_contract", {}) or {}
    if contract:
        lines.append("## Patch contract")
        lines.append("")
        lines.append(f"- Focus metric: {contract.get('focus_metric', '')}")
        lines.append(f"- Target: {contract.get('target', '')}")
        lines.append(f"- Single hypothesis: {contract.get('single_hypothesis', '')}")
        lines.append(
            "- Must-preserve rows: "
            + str(len(contract.get("must_preserve_news_ids", []) or []))
        )
        lines.append("")
...
def analyze_error_files_with_llm(
    ...
    slice_log_json: str | Path | None = None,
    focus_metric: str = "",
    ...
    try:
        ...
        merged_df = pd.DataFrame()
        if merged_csv:
            merged_path = Path(merged_csv).resolve()
            if merged_path.exists():
                merged_df = read_csv_file(merged_path)
        slice_diagnostics = {}
        if slice_log_json:
            slice_log_path = Path(slice_log_json).resolve()
            if slice_log_path.exists():
                slice_log_data = json.loads(slice_log_path.read_text(encoding="utf-8-sig"))
                slice_diagnostics = {
                    "type_mismatch_breakdown": slice_log_data.get(
                        "type_mismatch_breakdown", {}
                    ),
                    "primary_policy_counterfactual": slice_log_data.get(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--slice-log-json", type=str, default="")
    parser.add_argument("--focus-metric", type=str, default="")
    parser.add_argument("--merged-csv", type=str, default="")
...
if __name__ == "__main__":
    ...
    default_version_dir = resolve_versions_root(project_root) / "v1"
    ...
    result = analyze_error_files_with_llm(
        ...
        slice_log_json=args.slice_log_json or None,
        focus_metric=args.focus_metric,
        merged_csv=args.merged_csv or None,
    )
