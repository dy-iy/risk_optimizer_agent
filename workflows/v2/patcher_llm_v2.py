...
def compact_sample_case(case: dict) -> dict:
    return {
        ...
        "gold_risk_label": case.get("gold_risk_label", ""),
        "rule_risk_label": case.get("rule_risk_label", ""),
...
def compact_transition_risk_for_patcher(transition: dict) -> dict:
    ...
    return {
        ...
        "decision_dependencies": (transition.get("decision_dependencies", []) or [])[:6],
        "vulnerable_error_conversions": {
            "false_positive_to_type_mismatch_if_label_is_lowered": vulnerable.get(
                "false_positive_to_type_mismatch_if_label_is_lowered", 0
            ),
            "by_primary_type": (vulnerable.get("by_primary_type", []) or [])[:8],
            "by_dominant_scorer": (vulnerable.get("by_dominant_scorer", []) or [])[:8],
            "cases": [
                compact_sample_case(item)
                for item in (vulnerable.get("cases", []) or [])[:6]
                if isinstance(item, dict)
            ],
        },
        "must_preserve_sets": {
            "correct_low_with_typed_primary_rows": preserve.get(
                "correct_low_with_typed_primary_rows", 0
            ),
            "correct_low_with_typed_primary_cases": [
                compact_sample_case(item)
                for item in (preserve.get("correct_low_with_typed_primary_cases", []) or [])[:6]
                if isinstance(item, dict)
            ],
            "near_label_threshold_rows": preserve.get("near_label_threshold_rows", 0),
            "near_label_threshold_cases": [
                compact_sample_case(item)
                for item in (preserve.get("near_label_threshold_cases", []) or [])[:4]
                if isinstance(item, dict)
            ],
        },
    }
...
def compact_analysis_for_patcher(analysis_data: dict, limit: int = DEFAULT_ANALYSIS_LIMIT) -> dict:
    ...
    compact = {
        ...
        "slice_diagnostics": analysis_data.get("slice_diagnostics", {}),
        "optimization_contract": analysis_data.get("optimization_contract", {}),
        "transition_risk_analysis": compact_transition_risk_for_patcher(
            analysis_data.get("transition_risk_analysis", {}) or {}
        ),
        "patch_contract": analysis_data.get("patch_contract", {}),
...
def resolve_default_source_script(project_root: Path) -> Optional[Path]:
    ...
    candidates = [
        resolve_versions_root(project_root) / "v1" / "scripts" / "risk_labeler_v1.py",
    ]
    for p in candidates:
        if p.exists():
            return p
...
def resolve_default_analysis_json(project_root: Path) -> Path:
    ...
    return resolve_versions_root(project_root) / "v1" / "reports" / "analysis" / "risk_labeler_v1_analysis_llm.json"
...
def attach_patch_meta(
    ...
    report["meta"] = {
        ...
        "revision_attempt": bool(revision_feedback),
    }
    if compile_error:
...
if __name__ == "__main__":
    ...
    default_next_version_dir = resolve_versions_root(project_root) / "v2"
