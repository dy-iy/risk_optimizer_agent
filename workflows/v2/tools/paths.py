...
def resolve_versions_root(project_root: Path) -> Path:
    """Resolve the isolated version store for the active experiment.

    By default this remains ``<project>/versions`` for backward compatibility.
    Set ``RISK_VERSIONS_DIR`` to run a clean experiment without moving or
    overwriting historical versions. Relative paths are resolved from the
    project root.
    """
    configured = os.environ.get("RISK_VERSIONS_DIR", "").strip()
    if not configured:
...
def resolve_version_script(project_root: Path, version_name: str) -> Path:
    versions_root = resolve_versions_root(project_root)
    candidates = [
        versions_root / version_name / "scripts" / f"risk_labeler_{version_name}.py",
...
def build_paths(project_root: Path, current_version: str, next_version: str) -> dict[str, Path]:
    versions_dir = resolve_versions_root(project_root)
    ...
    return {
        ...
        "candidate_script": next_dir / "reports" / "patched" / f"risk_labeler_{next_version}_candidate.py",
        ...
        "impact_json": next_dir / "reports" / "comparisons" / f"{current_version}_vs_{next_version}_changed_rows.json",
        "changed_rows_csv": next_dir / "reports" / "comparisons" / f"{current_version}_vs_{next_version}_changed_rows.csv",
        "attempt_history_json": next_dir / "reports" / "orchestrations" / f"{current_version}_to_{next_version}_attempts.json",
        "relaxed_promotion_json": next_dir / "reports" / "orchestrations" / f"{current_version}_to_{next_version}_relaxed_promotion.json",
