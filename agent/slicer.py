...
def slice_errors(
    ...
    # 保存
    ensure_parent_dir(false_positive_csv)
    ensure_parent_dir(false_negative_csv)
    ensure_parent_dir(type_mismatch_csv)
    ensure_parent_dir(score_diff_top_csv)

    false_positive_df.to_csv(false_positive_csv, index=False, encoding="utf-8-sig")
    false_negative_df.to_csv(false_negative_csv, index=False, encoding="utf-8-sig")
    type_mismatch_df.to_csv(type_mismatch_csv, index=False, encoding="utf-8-sig")
    score_diff_top_df.to_csv(score_diff_top_csv, index=False, encoding="utf-8-sig")
...
if __name__ == "__main__":
    ...
    merged_csv = project_root / "reports" / "merged" / "risk_labeler_v1_merged.csv"
    false_positive_csv = project_root / "reports" / "errors" / "risk_labeler_v1_false_positive.csv"
    false_negative_csv = project_root / "reports" / "errors" / "risk_labeler_v1_false_negative.csv"
    type_mismatch_csv = project_root / "reports" / "errors" / "risk_labeler_v1_type_mismatch.csv"
    score_diff_top_csv = project_root / "reports" / "errors" / "risk_labeler_v1_score_diff_top.csv"
    log_json = project_root / "reports" / "errors" / "risk_labeler_v1_slice_log.json"
