...
NEG_POSITIVE_MOVE = ["小幅上涨", "微涨", "温和上涨", "企稳回升", "慢牛", "持续走高"]  # 只保留温和上涨
...
NEG_LIQ_LIQUIDITY_NON_RISK = ["代币化流动性", "流动性基金", "流动性产品", "清算价", "被动清算"]
...
# 新增：score_volatility 否定词（扩充财务利好）
...
KW_HACK = ["漏洞", "攻击", "被盗", "盗取", "重入", "闪电贷", "利用", "利用漏洞", "黑客", "exploit", "hacker", "入侵",
           ...
           "漏洞攻击", "社会工程", "路由器漏洞", "桥接漏洞", "钱包被盗", "盗取凭证"]
...
def score_hack(text):
    ...
    if has_any(text, KW_HACK):
        ...
        # 无金额但明确提到损失/攻击证据
        if has_any(text, ["损失", "被盗", "盗取", "窃取", "资金消失", "资金损失", "遭受攻击", "被利用"]):
            return 0.60
        # 仅含漏洞/攻击词但无损失佐证，视为讨论或历史提及
        return 0.35
    return 0.0
...
def score_liquidation(text):
    ...
    if has_any(text, NEG_LIQ_LIQUIDITY_NON_RISK):
        if has_any(text, KW_LIQ):
            usd = extract_usd_equiv(text)
            return clip01(0.25 if usd <= 1_000_000 else 0.30)
        return 0.0
...
# 8) 行情异常波动
KW_SHOCK = ["闪崩", "插针", "瀑布", "腰斩", "暴跌", "剧烈波动", "瞬间暴跌", "瞬间拉升"]   # 移除正面暴涨词
